	...
	def main(self, path1, path2, ruta_guardar_resultados):
		...
		for i in range(len(v_max)):
			...
			if indice_inicio_resp-1000>=0:
				...
				st_datos_resp=self.senal01[indice_inicio_resp-500: (indice_inicio_resp)+int(len(self.senal02)/2)]
				...
				print '-----------------------------'
 				if aux_m<time_m:
 					for j in range(aux_s, 60):
 						...
 						nombre_guardar_aux=inicio_genera.main(path1, str(time_h), str(aux_m), str(j))
						verifica_nombre=nombre_path_resultados+'/'+nombre_guardar_aux
						#print 'opcion 1'
						#print 'verifica: ', verifica_nombre
					...
					if resp_archivo_existe1 !=-1:
						for j in range(time_s+20):
 							#ruta=inicio_genera.main(path1, str(time_h), str(time_m), str(time_s))
 							nombre_guardar_aux=inicio_genera.main(path1, str(time_h), str(time_m), str(j))
							verifica_nombre=nombre_path_resultados+'/'+nombre_guardar_aux
							#print 'opcion 1'
							#print 'verifica: ', verifica_nombre
							if os.path.isfile(verifica_nombre):
				...
				if aux_mm>time_m:
					...
					if resp_archivo_existe3 != -1:		
						for j in range(aux_ss):
 							nombre_guardar_aux=inicio_genera.main(path1, str(time_h), str(aux_mm), str(j))
							verifica_nombre=nombre_path_resultados+'/'+nombre_guardar_aux
							#print 'opcion 3'
							#print 'verifica: ', verifica_nombre
							if os.path.isfile(verifica_nombre):
					...
					resp_archivo_existe3=1
				print 'respuesta 1', resp_archivo_existe1
				print 'respuesta 2', resp_archivo_existe2
				print 'respuesta 3', resp_archivo_existe3
				...
				if resp_archivo_existe1 == 1 and resp_archivo_existe2 == 1 and resp_archivo_existe3 == 1:
					...
					if respuesta_picker != -1:
						...
						#print 'time_m', time_m
						#print 'time_s', time_s
						#indice_inicio_resp= ((((time_h*60)+time_m)*60)+time_s)*100
						#print indice_inicio_resp
						...
						#stZ=obspy.read(path1)
						stE=obspy.read(path_HHE)
						stN=obspy.read(path_HHN)
						self.senalE=stE[0].data
						self.senalN=stN[0].data
						#self.senalZ=stZ[0].data

						# Se generar los nuevos intervalos de datos
						#stZ_new=self.senalZ[indice_inicio_resp-1000: (indice_inicio_resp)+800]
						stE_new=self.senalE[indice_inicio_resp-500: (indice_inicio_resp)+int(len(self.senal02)/2)]
						stN_new=self.senalN[indice_inicio_resp-500: (indice_inicio_resp)+int(len(self.senal02)/2)]

						#Se asiganan los nuevos intervalos de datos HHZ
						#stZ[0].stats.sac.npts=len(stZ[0].data)
						#stZ[0].data=stZ_new
						#stZ[0].stats.sac.nzhour=time_h
						#stZ[0].stats.sac.nzmin=time_m
						#stZ[0].stats.sac.nzsec=time_s
						#stZ[0].stats.starttime=time_inicio
						...
						#opcion=0 para guardar los datos en archivo txt y  regresa el dia en el que se encontro
						opcion=0
						Inicia_guardar_archivos.crea_nombre(path1, path2, nombre_path_resultados, nombre_guardar,indice_inicio_resp-1000, (indice_inicio_resp)+len(self.senal02), v_max[i], opcion, respuesta_picker,respuesta_picker_E, respuesta_picker_N )	
